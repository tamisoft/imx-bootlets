...
int PowerPrep_ConfigurePowerSource_ovl_set_vdd5v( void );
#endif
...
*/
#ifdef BOARD_IMX23_DWAMS
#define VBUS_CURRENT_LIMIT 0
//#define CONFIG_SET_VDD5V_INPUT
#else
...
#ifdef BOARD_IMX23_DWAMS
int PowerPrep_dwams_init()
{
	int val;
	
	printf( "PowerPrep_dwams_init\r\n" );

#ifdef CONFIG_SET_VDD5V_INPUT
	/*VBUS_EN as gpio - output == 1 */
	HW_PINCTRL_MUXSEL2_WR(HW_PINCTRL_MUXSEL2_RD() | BF_PINCTRL_MUXSEL2_BANK1_PIN15(3));
	HW_PINCTRL_DOUT1_WR(HW_PINCTRL_DOUT1_RD() | (1<<15));
	HW_PINCTRL_DOE1_WR(HW_PINCTRL_DOE1_RD() | (1<<15));
#endif	

	/* RESET_BT as gpio - output == 0 */
	HW_PINCTRL_MUXSEL3_WR(HW_PINCTRL_MUXSEL3_RD() | BF_PINCTRL_MUXSEL3_BANK1_PIN20(3));
	HW_PINCTRL_DOUT1_WR(HW_PINCTRL_DOUT1_RD() & ~(1<<20));
//	HW_PINCTRL_DOUT1_WR(HW_PINCTRL_DOUT1_RD() | (1<<20));
	HW_PINCTRL_DOE1_WR(HW_PINCTRL_DOE1_RD() | (1<<20));

#ifdef CONFIG_SET_VDD5V_INPUT
	PowerPrep_Delay(1000*10);
#endif	
...
#ifdef BOARD_IMX23_DWAMS
 #ifdef CONFIG_SET_VDD5V_INPUT
	PowerPrep_ConfigurePowerSource_ovl_set_vdd5v();
 #else
...
int PowerPrep_ConfigurePowerSource_ovl_set_vdd5v( void )
{
	...
	*/
	...
	//if(!IsVdd5vGtVddio())
	{
		...
		*/
 		bBatteryGood = true;
		printf("Boot from battery\r\n");
		
		if(IsVdd5vGtVddio())
			printf("DWAMS 5v input set by SW and detected correctly\r\n");
		else
			printf("DWAMS 5v input NOT detected !!!\r\n");
	...
	*/
	...
	printf("Charge pump feeds the VDD5V. Limit the 4p2 current to 0mA\r\n");
